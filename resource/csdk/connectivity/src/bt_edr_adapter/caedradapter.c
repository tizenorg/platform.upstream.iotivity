...
CAResult_t CANotifyEDRDeviceDiscoveryDone()
{
    ...
    result = CAQueueingThreadSendSignal(g_sendQueueHandle);
    if (CA_STATUS_OK != result)
    {
        OIC_LOG_V(ERROR, EDR_ADAPTER_TAG, "CAQueueingThreadSendSignal() Fail(%d)", result);
        return CA_STATUS_FAILED;
    }
                             ...
                             uint32_t dataLength, uint32_t *sentLength)
{
    ...
    else
    {
        ...
        {
            // Should add data to send thread before call bt_adapter_start_device_discovery()
            // cause of syncronization issue with bt discovery.
