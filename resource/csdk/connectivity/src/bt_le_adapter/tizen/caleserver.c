...
CAResult_t CAAddNewLEServiceInGattServer(const char *serviceUUID)
{
    ...
    VERIFY_NON_NULL(serviceUUID, TAG, "Param serviceUUID is NULL");
...
CAResult_t CARegisterLEServicewithGattServer(const bt_gatt_h svcPath)
{
    ...
    VERIFY_NON_NULL(svcPath, TAG, "Param svcPath is NULL");
                                               ...
                                               uint32_t charValueLen)
{
    ...
    VERIFY_NON_NULL(charValue, TAG, "Param charValue is NULL");
    VERIFY_NON_NULL(address, TAG, "Param address is NULL");
